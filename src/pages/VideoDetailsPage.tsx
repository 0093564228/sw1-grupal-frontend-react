...
export default function VideoDetailsPage() {
  ...
  const loadAlbums = useCallback(async (userId: number) => {
    try {
  ...
  }, [jobId, user, loadVideo, loadAlbums]);

  const handleMove = async () => {
    if (!video || !selectedAlbumId) return;

    try {
      await albumService.moveVideo(
        video.job_id,
        Number(selectedAlbumId),
      );
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="bg-gray-900 rounded-xl overflow-hidden shadow-2xl border border-gray-700">
          <ResultsPage
            ...
            albumName={
              albums.find((a) =>
                a.videos?.some((v) => v.job_id === video.job_id),
              )?.name
            }
          />
        </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
          <div className="bg-gray-800 p-6 rounded-lg shadow-xl w-96 border border-gray-600">
            ...
            >
              ...
              {albums
                .filter((a) =>
                  a.videos?.every((v) => v.id !== video.id),
                ) // Filtrar álbum actual si ya lo tuviéramos en contexto (opcional)
                .map((album) => (
