...
export default function AlbumVideosPage() {
  ...
  const handleUpdateVideo = async (job_id: string, newName: string) => {
    try {
      const updatedVideo = await albumService.updateVideo(
        job_id,
        newName,
      );
      setVideos(
        videos.map((v) => (v.job_id === job_id ? updatedVideo : v)),
      );
    } catch (error) {
  ...
  const handleEditAlbum = async () => {
    ...
    if (newName && newName !== album.name) {
      try {
        const updated = await albumService.updateAlbum(album.id, {
          name: newName,
        });
        setAlbum(updated);
      } catch (error) {
  ...
  const handleDeleteAlbum = async () => {
    ...
    if (
      confirm(
        `¿Estás seguro de eliminar el álbum "${album.name}" y todos sus videos? Esta acción no se puede deshacer.`,
      )
    ) {
      try {
          ...
          <div className="flex items-start gap-6">
            ...
            <div>
              ...
              <p className="text-gray-400 text-sm mt-1">
                {videos.length}{" "}
                {videos.length === 1 ? "video" : "videos"} • Creado el{" "}
                {album
                ...
                .map((video) => (
                  ...
                  >
                    ...
                    <div
                      ...
                      onClick={() =>
                        navigate(
                          `/videos/${encodeURIComponent(video.job_id)}`,
                        )
                      }
                    >
                      {video.job_id ? (
                        ...
                        <div className="flex space-x-1">
                          ...
                          <button
                            onClick={(e) => {
                              ...
                              if (
                                window.confirm(
                                  `¿Estás seguro de eliminar el video "${video.name}"? Esta acción no se puede deshacer.`,
                                )
                              ) {
